...
Currently the async/DMA version doesn't seem to work properly on the STM32083RC. I had to switch to a different board, and a working version can be found in the [rp235x](../rp235x/src/bin/arducam-async.rs) example. In particular, I2C async works but there is some issue with the SPI Async/DMA interface.
